...
st.set_page_config(page_title="PointKey MVP v5", layout="wide")
...
def reset_point(i: int) -> None:
    st.session_state.scenarios[i]["x"] = 0
    st.session_state.scenarios[i]["y"] = 0


def nudge_point(i: int, dx: int, dy: int) -> None:
    sc = st.session_state.scenarios[i]
    step = int(sc.get("nudge_step", 10))
    ...
    st.session_state.scenarios[i]["x"] = new_x
    st.session_state.scenarios[i]["y"] = new_y
...
def joystick_controls(i: int) -> None:
    ...
    sc["nudge_step"] = st.select_slider(
        "이동 단위(px)",
        options=STEP_OPTIONS,
        value=int(sc.get("nudge_step", 10)),
        key=f"step_{i}"
    )
...
for i, sc in enumerate(st.session_state.scenarios):
    ...
    with left:
        ...
        if image is not None:
            ...
            st.image(preview, caption=f"{sc['image_name']} | {sc['img_w']} x {sc['img_h']} | 포인트: ({sc['x']}, {sc['y']})", use_container_width=True)
        else:
            sc["image_name"] = ""
...
st.download_button(
    ...
    file_name="pointkey_generated_v5.ahk",
    mime="text/plain",
)
