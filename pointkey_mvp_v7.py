...
st.set_page_config(page_title="PointKey MVP v7", layout="wide")
...
def ensure_widget_state(i: int) -> None:
...
def sync_from_widgets(i: int) -> None:
    sc = st.session_state.scenarios[i]
    x_key = f"x_direct_{i}"
    y_key = f"y_direct_{i}"
    step_key = f"step_{i}"

    x = int(st.session_state.get(x_key, sc["x"]))
    y = int(st.session_state.get(y_key, sc["y"]))
    ...
    sc["x"] = x
    sc["y"] = y
    sc["nudge_step"] = int(st.session_state.get(step_key, sc.get("nudge_step", 10)))
...
def reset_point(i: int) -> None:
    ...
    sc["x"] = 0
    sc["y"] = 0
    st.session_state[f"x_direct_{i}"] = 0
    st.session_state[f"y_direct_{i}"] = 0
...
def nudge_point(i: int, dx: int, dy: int) -> None:
    ...
    sc["y"] = int(new_y)
    sc["nudge_step"] = step
    st.session_state[f"x_direct_{i}"] = int(new_x)
    st.session_state[f"y_direct_{i}"] = int(new_y)
...
st.download_button(
    ...
    file_name="pointkey_generated_v7.ahk",
    mime="text/plain",
)
